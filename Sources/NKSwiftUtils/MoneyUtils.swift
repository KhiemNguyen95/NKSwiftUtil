...
final class MoneyUtils {
    ...
    ///dislay string with format currency
    ...
    static func validInput(_ data: String?, replacementString string: String) -> Bool {
        ...
        if indexDecimalSeparator != nil && (data?.count ?? 0) >= (indexDecimalSeparator! + 3) && string != "" {
            // khong che 2 chu so thap phan
            return false
        }
        if string == MoneyUtils.decimalSeparator {
            // khong che nhap 1 ký tu thap phan
            return indexDecimalSeparator == nil
        }
